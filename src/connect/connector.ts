...
>(uri: string, config: APIConnectorConfig<URLParams, ReqType | undefined, ResType>) {
    function getHeader() {
        if (!config.needAuth) return { 'Content-Type': 'application/json' }
    ...
    return {
        ...
        request(urlParams: URLParams, reqBody?: ReqType, customHeader: Record<string, string> = {}) {
            return new Promise<ResType>((ok, error) => {
                if (isDev && config.mockHandler) {
                    setTimeout(async () => {
                        const responseData = await config.mockHandler(urlParams)
                        if (!responseData) throw new Error("Cannot get message")
                        ok(responseData)
                        return
                    }, 1000)
                    return
                ...
                console.log({ ...getHeader(), ...customHeader })
                fetch(API_URI + reqUri, {
                    method: config.method,
                    body: config.method === 'GET' ? undefined : JSON.stringify(reqBody),
